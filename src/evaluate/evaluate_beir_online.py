...
def _get_scores_for_adv_passage(
    ...
    assert sim_func_name in [
        "cos_sim",
        "dot",
    ], f"Unknown similarity function: {sim_func_name}"

    adv_results = {
        qid: {} for qid in attacked_qids
    }  # {qid -> {adv_pid1: score1, adv_pid1: score2, ...}}

    q_embs = torch.stack(
        [qid_to_emb[qid] for qid in attacked_qids]
    )  # shape: (n_queries, emb_dim)
    sim_to_adv = torch.zeros(
        len(attacked_qids), len(adv_passage_vecs), device="cpu"
    )  # in CPU to avoid GPU OOM
    ...
    for i in range(0, len(adv_passage_vecs), batch_size):
        ...
        if sim_func_name == "cos_sim":
            ...
            )  # shape: (n_queries, n_adv_p_batch)
        elif sim_func_name == "dot":
            sim_to_adv_batch = torch.matmul(
                q_embs, adv_passage_vecs_batch.t()
            )  # shape: (n_queries, n_adv_passages)
...
def _inject_adv_results_to_results(
    results: Dict[
        str, Dict[str, float]
    ],  # the results dict to inject the scores into (it is deep-copied)
...
def _evaluate_with_adv_passage(
    results: Dict[
        str, Dict[str, float]
    ],  # the full retrieval results to evaluate with (after inserting the adv scores)
    pid_advs: List[str],  # the pid of the adversarial passages to evaluate
    attacked_gold_qrels: Dict[
        str, Dict[str, int]
    ],  # qid -> {pid1: gold_rank, pid2: gold_rank, ...}
    # metrics:
    ...
    results = {
        qid: results[qid] for qid in attacked_gold_qrels.keys()
    }  # filter `results` by attacked queries
    attacked_adv_qrels = {
        qid: {adv_pid: 1 for adv_pid in pid_advs} for qid in attacked_gold_qrels.keys()
    }

    # Evaluate classic metrics on benign data (i.e. relative to golden passages):
    ndcg, _map, recall, precision = evaluation.EvaluateRetrieval.evaluate(
        attacked_gold_qrels, results, k_values
    )
    mrr = evaluation.EvaluateRetrieval.evaluate_custom(
        attacked_gold_qrels, results, k_values, metric="mrr"
    )
    logger.info(
        f"\n [BENIGN PASSAGES] \n {ndcg}, \n {_map}, \n {recall}, \n {precision}, \n {mrr} \n "
    )
    # Evaluate relative to adversarial passages (as if the goal is to reach them):
    ndcg_adv, _map_adv, recall_adv, precision_adv = (
        evaluation.EvaluateRetrieval.evaluate(attacked_adv_qrels, results, k_values)
    )
    mrr_adv = evaluation.EvaluateRetrieval.evaluate_custom(
        attacked_adv_qrels, results, k_values, metric="mrr"
    )
    logger.info(
        f"\n [ADV PASSAGES] \n {ndcg_adv}, \n {_map_adv}, \n {recall_adv}, \n {precision_adv}, \n {mrr_adv} \n "
    )
    ...
    for i, q in enumerate(attacked_gold_qrels.keys()):
        pid_and_score = sorted(
            list(results[q].items()), key=lambda x: x[1], reverse=True
        )
        gold_cou = 0
        ...
        for rank, (pid, score) in enumerate(pid_and_score):
            if pid in curr_adv_qrels:
                adv_ranks[i] = min(
                    [rank, adv_ranks[i]]
                )  # we care about the first time we find the adv-passage
        ...
        if (
            gold_ranks[i] == max_evaluated_rank
        ):  # if the golden-passage was not found in the results (quite odd...)
            logger.info(
                f">> {q=}, golden-{attacked_gold_qrels[q]=}, {'not found in results'}"
            )
        if (
            adv_ranks[i] == max_evaluated_rank
        ):  # if the adv passage was not found in the results
    ...
    logger.info(
        f"average adversary wins over golden: {(adv_ranks < gold_ranks).mean()}"
    )
    ...
    metrics = {
        ...
        f"adv_appeared@10{metrics_suffix}": adv_ranks[adv_ranks < 10].size
        / adv_ranks.size,
        ...
        f"adv_wins_over_golden{metrics_suffix}": (
            adv_ranks < gold_ranks
        ).mean(),  # main metric, higher means better
        ...
        metrics.update(
            {
                ...
                f"gold_appeared@10{metrics_suffix}": gold_ranks[gold_ranks < 10].size
                / gold_ranks.size,
            ...
            metrics.update(
                {
                    ...
                    f"adv_appeared@{k}{metrics_suffix}": adv_ranks[adv_ranks < k].size
                    / adv_ranks.size,
                }
            )
    return metrics
...
def full_evaluation_with_adv_passage_vecs(
    ...
    pid_advs = [
        f"__adv{i}__" for i in range(len(adv_passage_vecs))
    ]  # set available pids to name adversarial passages
    if isinstance(adv_passage_vecs, list):
        adv_passage_vecs = torch.stack(
            adv_passage_vecs, dim=0
        )  # shape: (n_adv_passages, emb_dim)
    # set adv_passage_ids to the same device as `qid_to_emb`:
    adv_passage_vecs = adv_passage_vecs.to(
        qid_to_emb[list(qid_to_emb.keys())[0]].device
    )
...
def get_result_list_for_query(
    ...
    data_split: str,
    data_portion: float,
    adv_passage_texts: List[str] = None,
    adv_passage_toks: List[List[int]] = None,
    ...
    if adv_passage_texts is not None:
        ...
        max_length = max(
            max(len(seq) for seq in adv_passage_toks), model.tokenizer.model_max_length
        )
        input_ids = torch.full(
            (len(adv_passage_toks), max_length),
            model.tokenizer.pad_token_id,
            dtype=torch.long,
        )
        attention_mask = torch.zeros(
            (len(adv_passage_toks), max_length), dtype=torch.long
        )
        for i, seq in enumerate(adv_passage_toks):
