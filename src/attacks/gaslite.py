...
def gaslite_attack(
    ...
    **kwargs,  # additional kwargs to pass to the model for calculating the loss (e.g., labels)
    ...
    switch_to_gcg_in_step = (
        beam_search_config.get("switch_to_gcg_in_step", None) or math.inf
    )
    ...
    if k_candidates is None and beam_search_config["perform"]:
        k_candidates = (
            beam_search_config["n_cand"] * 2
        )  # default value for k_candidates

    token_ids_to_ignore = model.get_special_token_ids()
    logger.info(
        f"Attack ignores {len(token_ids_to_ignore) / vocab_size :.3f}% of the tokens."
    )

    # >> Initialize the fluency scorer
    flu_model = initialize_fluency_model(
        fluency_model_name, batch_size=128
    )  # TODO generalize
    if (
        (
            flu_alpha != 0 or flu_alpha_inc.get("filter_to_n_most_readable", None)
        )  # if you use fluency
        and model.tokenizer.__class__
        != flu_model.tokenizer.__class__  # and we have tokenizer mismatch
    ):
        raise ValueError(
            "Fluency model's tokenizer and the model's tokenizer must match."
        )
    ...
    if trigger_len == 0:
        logger.info(
            "Trigger is empty; stopping the attack."
        )  # we allow this for experiments
        best_input_ids = input_ids
        best_loss = torch.tensor(-1)
    else:
        for i in trange(n_iter, desc="Attacking with GASLITE..."):
            if (
                "chunk_robustness_method" in kwargs
                and kwargs["chunk_robustness_method"] == "monte_carlo"
            ):
                stop_index = random.randint(
                    trigger_slice.start + 1, trigger_slice.start + trigger_len
                )
                monte_carlo_indexes.append(stop_index)
                print("Updated Monte Carlo Indexes", monte_carlo_indexes)
                trigger_slice = slice(trigger_slice.start, stop_index)
            tmp_trigger_len = trigger_slice.stop - trigger_slice.start
            ...
            if n_grad > 1:
                ...
                if attack_variant == "arca":
                    ...
                    _fixed_pos = (
                        i % (trigger_slice.stop - trigger_slice.start)
                        + trigger_slice.start
                    )  # ARCA's `_fixed_pos`
                    ...
                    input_ids_repeated[:, _fixed_pos] = torch.randint(
                        0, vocab_size, size=(n_grad,), device="cuda"
                    ...
                    random_tok_positions = torch.randint(
                        trigger_slice.start,
                        trigger_slice.stop,
                        size=(n_grad - 1,),
                        device="cuda",
                    ...
                    input_ids_repeated = input_ids.repeat(n_grad, 1)
                    input_ids_repeated[
                        torch.arange(len(input_ids), len(input_ids_repeated)),
                        random_tok_positions,
                    ] = torch.randint(
                        0, vocab_size, size=(n_grad - 1,), device="cuda"
                    ).repeat_interleave(
                        n_samples
                    )
                grad_inputs = {
                ...
                grad_inputs = inputs
                grad_kwargs = kwargs
            if "chunk_robustness_method" in kwargs:
                grad_kwargs["chunk_robustness_method"] = kwargs[
                    "chunk_robustness_method"
                ]
            ...
            if n_grad > 1:
                ...
                onehot_grads = onehot_grads.view(
                    n_grad, onehot_grads.shape[0] // n_grad, *onehot_grads.shape[1:]
                ).mean(dim=0)

            # 2. Scores are the average of the gradients
            scores = onehot_grads.mean(
                dim=0
            )  # (n_samples, targeted_sub_seq_len, vocab_size)

            # 3. Get the candidates for token flips
            filter_to_n_most_readable = flu_alpha_inc.get(
                "filter_to_n_most_readable", None
            )
            candidates = get_trigger_candidates(
                scores=scores,
                flu_scores=(
                    flu_model.calc_fluency_scores(
                        inputs={
                            "input_ids": input_ids,
                    ...
                    if filter_to_n_most_readable is not None
                    else None
                ),
                trigger_slice=trigger_slice,
            ...
            flipped_idx_and_id_lst = (
                []
            )  # each element is a tuple of (token_idx, token_id)
            ...
            if attack_variant == "autoprompt":
                ...
                token_indices_to_flip = np.random.randint(
                    trigger_slice.start, trigger_slice.stop, size=n_flips_to_gen
                ).astype(int)
                token_top_ids_indices = np.random.randint(
                    k_candidates, size=n_flips_to_gen
                )
                ...
                token_indices_to_flip = np.arange(
                    trigger_slice.start,
                    trigger_slice.stop,
                    step=(trigger_slice.stop - trigger_slice.start) / n_flips_to_gen,
                ).astype(int)
                token_top_ids_indices = np.random.randint(
                    k_candidates, size=n_flips_to_gen
                )
            ...
            for token_idx, token_top_ids_idx in zip(
                token_indices_to_flip, token_top_ids_indices
            ):
            ...
            flipped_input_ids_lst = torch.stack(
                flipped_input_ids_lst, dim=0
            ).cuda()  # (n_flipped, n_samples, seq_len)
            ...
            if beam_search_config["perform"] and i < switch_to_gcg_in_step:
                ...
                bs_params = dict(
                    ...
                    B=beam_search_config.get(
                        "B", 1
                    ),  # size ("width") of preserved set after its iter ("beam")
                    n_cand=beam_search_config[
                        "n_cand"
                    ],  # number of candidates to evaluate, per position
                    n_cand__sample_rate=beam_search_config.get(
                        "n_cand__sample_rate", 0.5
                    ),  # sample rate for the top-n_cand candidates
                    beam_indices=beam_search_config.get(
                        "beam_indices", None
                    ),  # indices to consider for beam-search
                )
                # Deduce the different n_cand's
                bs_params["n_cand__from_sample"] = int(
                    bs_params["n_cand"] * bs_params["n_cand__sample_rate"]
                )
                bs_params["n_cand__from_top"] = int(
                    bs_params["n_cand"] * (1 - bs_params["n_cand__sample_rate"])
                )
                bs_params["n_flip"] = (
                    bs_params["n_flip"] or 20
                )  # default value for n_flip
                if bs_params["n_flip"] > tmp_trigger_len:
                    bs_params["n_flip"] = int(
                        math.ceil(tmp_trigger_len * 0.67)
                    )  # make sure n_flips is not bigger than the trigger length
                ...
                if (
                    bs_params["beam_indices"] == "random_interval"
                ):  # for choosing only contiguous subsets
                    beam_interval_start = np.random.choice(
                        np.arange(
                            trigger_slice.start,
                            trigger_slice.stop - bs_params["n_flip"],
                        )
                    )
                    trigger_indices = np.arange(
                        beam_interval_start, beam_interval_start + bs_params["n_flip"]
                    )
                elif (
                    bs_params["beam_indices"] == "exhaust_interval"
                ):  # for deterministic contiguous subsets.
                    beam_interval_start = (
                        i
                        % (
                            trigger_slice.stop
                    ...
                    trigger_indices = sorted(
                        np.random.choice(
                            np.arange(trigger_slice.start, trigger_slice.stop),
                            size=bs_params["n_flip"],
                            replace=False,
                        )
                    )

                beam_pool = (
                    input_ids.clone().detach().unsqueeze(0)
                )  # [n_beams, n_samples, seq_len)
                for curr_token_idx in tqdm(
                    trigger_indices, desc="Performing Beam-Search..."
                ):

                    # List the possible flips to apply on the beam-pool
                    # Option 1: choose the top-n_cand candidates
                    token_top_ids_1 = torch.tensor(
                        candidates[curr_token_idx]["token_top_ids"][
                            : bs_params["n_cand__from_top"]
                        ]
                    ).cuda()
                    # Option 2: randomly choose from the top-n_cand candidates
                    token_top_ids_2 = torch.tensor(
                        random.choices(
                            candidates[curr_token_idx]["token_top_ids"][
                                bs_params["n_cand__from_top"] :
                            ],
                    ...
                    for (
                        beam_node
                    ) in beam_pool:  # for each beam -> expand via possible flips
                        beam_pool_next.append(
                            beam_node
                        )  # We always keep the original beam-node (i.e., no flip)
                        new_beam_nodes = (
                            beam_node.clone().detach().repeat(len(token_top_ids), 1, 1)
                        )
                        new_beam_nodes[
                            torch.arange(len(token_top_ids)).cuda(), :, curr_token_idx
                    ...
                    beam_pool = beam_pool_next[
                        losses.argsort(descending=True)[: bs_params["B"]]
                    ]
                    beam_pool_losses = losses[
                        losses.argsort(descending=True)[: bs_params["B"]]
                    ]
            ...
            if use_rephraser and i in [(n_iter // 3) * 2]:  # [UNUSED]
                ...
                curr_trigger_str = model.tokenizer.batch_decode(
                ...
                new_cos_sims = torch.nn.functional.cosine_similarity(
                    model.embed([curr_trigger_str]), model.embed(new_trigger_strs)
                )
                new_trigger_str = new_trigger_strs[new_cos_sims.argmax()]
                logger.info(
                    f"Rephrased the trigger (sim={new_cos_sims.max().item() :.4f}): "
                    f"{curr_trigger_str} -> {new_trigger_str}"
                )
                new_trigger_str = (
                    new_trigger_str * 2
                )  # makes sure the trigger is long enough

                # Tokenize, trim and update:
                new_input_ids = model.tokenizer(
                    new_trigger_str,
                    return_tensors="pt",
                    padding="max_length",
                    truncation=True,
                ).input_ids.cuda()
                new_input_ids[:, trigger_slice.stop] = (
                    model.tokenizer.sep_token_id
                )  # add the sep token
                new_input_ids[:, (trigger_slice.stop + 1) :] = (
                    model.tokenizer.pad_token_id
                )
                input_ids = new_input_ids

            text_inputs = model.tokenizer.batch_decode(
                input_ids, skip_special_tokens=True
            )
            other_metrics = _calc_other_metrics(
                model,
                input_ids,
                attention_mask,
                text_inputs,
            ...
            logger.info(
                f"<iter-{i}> loss: {loss}, {', '.join([f'{k}: {v}' for k, v in other_metrics.items()])}"
            )
            if log_to == "wandb":
                wandb.log(
                    {
                        "attack_loss": loss,
                        **other_metrics,
                        "adv_text": text_inputs[
                            0
                        ],  # logs the adversarial text; disable when not needed.
                    }
                )
            if log_to == "livelossplot":
                # print(loss)
                ...
                plotlosses.send()
            logger.info(
                "adv_text: " + text_inputs[0]
            )  # logs the adversarial text; disable when not needed.

            if (
                (flu_alpha == 0 and l2_alpha == 0 and loss > best_loss)  # no fluency
                or (
                    flu_alpha != 0 and i >= n_iter // 2 and loss > best_loss
                )  # for flu we start checking after half of the iterations
                or (
                    l2_alpha != 0 and i >= n_iter // 2 and loss > best_loss
                )  # for l2 we start checking after half of the iterations
            ...
            if (
                flu_alpha != 0
                and i >= n_iter // 2
                and other_metrics["fluency_score"] > best_flu
            ):
                best_flu = other_metrics["fluency_score"]
                best_flu_input_ids = input_ids
                logger.info(
                    "Fluency score is improved; best-fluency instance was updated."
                )
            ...
            # TR
            if checkpoint_every_n and (i + 1) % checkpoint_every_n == 0:
                best_input_ids_dict[i + 1] = best_input_ids
            # TR
            if time_limit_in_seconds is not None and time_limit_in_seconds < (
                time.time() - start_time
            ):
                logger.info(
                    f"Time limit ({time_limit_in_seconds} sec) reached; stopping the attack."
                )
                break
    trigger_slice = slice(trigger_slice.start, trigger_slice.start + trigger_len)
    # Calculate the final metrics of the best instances
    final_metrics = {}

    if flu_alpha != 0:  # Evaluate the best-flu instance and log the metrics
        text_inputs = model.tokenizer.batch_decode(
            best_flu_input_ids, skip_special_tokens=True
        )
        other_metrics = _calc_other_metrics(
            model,
            best_flu_input_ids,
        ...
        final_metrics.update(
            {
                "best_flu_instance_text": text_inputs[0],
    ...
    if "chunk_robustness_method" in kwargs and kwargs["chunk_robustness_method"]:
        print("Final Monte Carlo Indexes", monte_carlo_indexes)
    if not checkpoint_every_n:
        best_input_ids_dict[n_iter] = best_input_ids
    return (
        best_input_ids_dict[n_iter] if return_last else best_input_ids_dict
    ), final_metrics


def _calc_other_metrics(
    model, input_ids, attention_mask, text_inputs, trigger_slice, flu_model, emb_targets
):
    ...
    return dict(
        ...
        cos_sim_adv_target=torch.nn.functional.cosine_similarity(emb, emb_targets)
        .mean()
        .item(),
        cos_sim_adv_target_on_text=torch.nn.functional.cosine_similarity(
            model.embed(texts=text_inputs), emb_targets
        )
        .mean()
        .item(),
...
def _token_gradients_batch(
    ...
    trigger_slice: slice,
    inputs: dict,
    backprop_batch_size: int = 150,
    flu_model=None,
    flu_alpha: float = 0,
    l2_alpha: float = 0,
    **kwargs,
) -> torch.Tensor:
    """Calculate token gradients with efficient memory handling."""
    ...
    for i in range(n_batches):
        ...
        batch_inputs = {
            k: v[batch_slice].to("cuda", non_blocking=True) for k, v in inputs.items()
        }
        batch_kwargs = {
            k: (
                v[batch_slice].to("cuda", non_blocking=True)
        ...
        grads.append(batch_grads.cpu())  # Move to CPU immediately to free GPU memory
...
def _decode_and_retokenize(
    ...
    flipped_input_ids_lst: List[torch.Tensor],
    flipped_idx_and_id_lst: list,
    **kwargs,
) -> Tuple[List[torch.Tensor], List[Any]]:
    """Efficiently decode and retokenize flipped inputs."""
    pad_token_id = model.tokenizer.pad_token_id
    old_decoded_str = model.tokenizer.batch_decode(
        input_ids.cpu(), skip_special_tokens=True
    )

    new_flipped_input_ids_lst, new_flipped_idx_and_id_lst = [], []
    for flipped_input_ids, flipped_idx_and_id in zip(
        flipped_input_ids_lst, flipped_idx_and_id_lst
    ):
        new_decoded_str = model.tokenizer.batch_decode(
            flipped_input_ids.cpu(), skip_special_tokens=True
        )
        new_flipped_input_ids = model.tokenizer(
            new_decoded_str, return_tensors="pt", padding="max_length", truncation=True
        ).input_ids.to("cuda", non_blocking=True)

        # Efficient validation logic
        if old_decoded_str != new_decoded_str and len(new_flipped_input_ids) == len(
            flipped_input_ids
        ):
    ...
    logger.info(
        f"Ratio of flips kept: {len(new_flipped_input_ids_lst) / len(flipped_input_ids_lst):.2f}"
    )
...
def _calc_losses_of_flip(
    ...
    attention_mask: torch.Tensor,
    trigger_slice: slice,
    flipped_input_ids_lst: torch.Tensor,
    flipped_idx_and_id_lst: list,
    flu_model=None,
    flu_alpha: float = 0,
    l2_alpha: float = 0,
    **kwargs,
    ...
    """Calculate losses for flips with efficient memory usage."""
    n_flipped, _, seq_len = flipped_input_ids_lst.shape
    calc_loss_for_eval_kwargs = _hack_to_enlarge_kwargs(kwargs, n_flipped)

    with torch.no_grad():  # Use no_grad to reduce memory usage for loss calculation
        losses = (
            model.calc_loss_for_eval(
                inputs={
                    "input_ids": flipped_input_ids_lst.view(-1, seq_len),
