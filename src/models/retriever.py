...
class RetrieverModel:
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_hf_name, trust_remote_code=_trust_remote_code
        )
        ...
        self.model_encoder: sentence_transformers.models.Transformer = self.model[
            0
        ].auto_model  # TODO generalize
        self.model_pooler: sentence_transformers.models.Pooling = self.model[
            1
        ]  # TODO generalize
        ...
        if (
            model_hf_name == "sentence-transformers/gtr-t5-base"
        ):  # HACK to generalize to T5 tokenizer
        ...
        for (
            param
        ) in self.model.parameters():  # prevent the model from updating the gradients
        ...
        def _sum_of_high_margin(x, targets, anchors):
            ...
            assert (
                anchors is not None
            ), "Must provide `emb_anchors` for `sum_of_high_margin` loss."
            ...
            return -1 * torch.nn.functional.relu(
                sim_x_to_anchors - sim_x_to_target + eps
            )  # `-1` since adversary would like to minimize this loss
    ...
    def embed(self, texts: List[str] = None, inputs: dict = None, to_cpu: bool = False):
        ...
        assert (
            texts is not None or inputs is not None
        ), "Either `texts` or `inputs` must be provided."
        ...
        for i in trange(n_batches, desc="Embedding..."):
            ...
            with torch.no_grad():
                ...
                batch_inputs["token_embeddings"] = model_output[
                    0
                ]  # add the model's token embedding to the `inputs`
                ...
                sentence_embeddings.append(
                    sentence_embedding.cpu() if to_cpu else sentence_embedding
                )

        sentence_embeddings = torch.cat(sentence_embeddings, dim=0)
        return sentence_embeddings

    def calc_loss_for_grad(
        self,
        inputs_embeds,
        ...
        **kwargs,
        ...
        inputs["token_embeddings"] = model_output[
            0
        ]  # add the model's token embedding to the `inputs`
    ...
    def calc_loss_for_eval(
        ...
        **kwargs,
        ...
        n_samples = inputs["input_ids"].shape[0]
        _max_batch_size = (
            n_samples  #  self.max_batch_size [BATCH IS CURRENTLY DISSABLED]
        )
        ...
        for i in trange(n_batches, desc="Calculating loss..."):
            ...
            batch_emb_anchors = (
                emb_anchors[batch_slice] if emb_anchors is not None else None
            )

            # 3. Calculate loss over batch
            with torch.no_grad():
                batch_losses = self.adv_loss_func(
                    batch_emb_inputs, batch_emb_targets, batch_emb_anchors
                )
    ...
    def get_special_token_ids(self) -> List[int]:
        ...
        special_tokens = [
            getattr(self.model.tokenizer, attr)
            for attr in [
                "cls_token",
                "sep_token",
                "mask_token",
        ...
        special_token_ids = [
            self.model.tokenizer.convert_tokens_to_ids(token)
