...
def attack_ret(
    ...
    **kwargs,
    ...
    if cover_alg.get("concept_qids", None) is not None:
        ...
        cover_alg["concept_train_qids"] = concept_qids[
            : int(len(concept_qids) * cover_alg["concept_portion_to_train"])
        ]
        cover_alg["filter_in_qids_name"] = (
            f"heldin-concept-{cover_alg['concept_name']}-{cover_alg['concept_portion_to_train']}"
        )
    ...
    if (
        "toxic_passage" in cover_alg
    ):  # if the toxic passage is predefined (e.g., by concept)
        if type(cover_alg["toxic_passage"]) is dict:
            mal_info = cover_alg["toxic_passage"][kwargs["mal_info_length"]]
        else:
            mal_info = cover_alg["toxic_passage"]

    # Fetch targeted queries and (golden) passage
    Q, Qid, Pgold, Pgold_id = get_qs_and_golden_ps(
        qp_pairs_dataset,
        query_choice,
        target_choice,
    ...
    emb_targets, centroid_vec = calc_objective(
        model, Q, sim_func_name, target_choice, centroid_vecs[cluster_idx]
    )
    ...
    P_adv = P_adv.to("cuda")
    logger.info(
        f"[[decoded-trigger-slice]]{model.tokenizer.decode(P_adv['input_ids'][0][trigger_slice])}"
    )
    logger.info(
        f"[[full decoded-passage]]{model.tokenizer.decode(P_adv['input_ids'][0])}"
    )
    ...
    def _decode(_input_ids, is_clean=True):
        return model.tokenizer.decode(
            _input_ids,
            skip_special_tokens=is_clean,
            clean_up_tokenization_spaces=is_clean,
        )
    ...
    adv_tokens_list_after_attack = adv_tokens_list_after_attack[
        : adv_tokens_list_after_attack.index(model.tokenizer.pad_token_id)
    ]  # remove padding
    ...
    logger.info(
        f"Decoding `adv_pass_after_attack` with special tokens: "
    ...
    if not (
        ...
        logger.warning(
            "[WARNING] Note that the `adv_pass_before_attack` cannot be reversed correctly and "
            "might degrade the attack performance."
        )
    orig = P_adv["input_ids"][0].cpu()
    dec = model.tokenizer(
        adv_pass_after_attack,
        return_tensors="pt",
        padding="max_length",
        truncation=True,
    )["input_ids"][0].cpu()
    if not (orig == dec).all():
        logger.warning(
            "[WARNING] Note that the `adv_pass_after_attack` cannot be reversed correctly and "
            "might degrade the attack performance."
        )
...
def get_qs_and_golden_ps(
    ...
    if query_choice == "cluster" or target_choice == "predefined_centroid":
        ...
        query_choice == "single_random"
    ):  # a single query should be chosen, according to the random seed
        ...
        query_choice, list
    ):  # query_choice is a list of pre-chosen query-ids
        for idx, qid in enumerate(qp_pairs_dataset["query_id"]):
            if qid in query_choice:
                Q.append(qp_pairs_dataset["query"][idx])
                Qid.append(qp_pairs_dataset["query_id"][idx])
    ...
    if n_samples is not None:
        assert n_samples <= len(
            Q
        ), f"{n_samples=} must be smaller/eq to the number of queries in the cluster ({len(Q)})"
        Q, Qid, Pgold, Pgold_id = (
            Q[:n_samples],
            Qid[:n_samples],
            Pgold[:n_samples],
...
def calc_objective(model, Q, sim_func, target_choice, predefined_centroid_vec):
    # calc centroid:
...
def initialize_p_adv(
    ...
    if adv_passage_init == "lm_gen":
        ...
        gen_texts = generator(
            ...
            max_new_tokens=trigger_len * 2,
            num_return_sequences=5,
        )
        # Choose the generated text of maximal length
        gen_text = max(gen_texts, key=lambda x: len(x["generated_text"]))[
            "generated_text"
        ]
        ...
        trigger_init = " ".join(
            random.choices(qp_pairs_dataset["passage"], k=10)
        )  # repeat to match the trigger len
    elif adv_passage_init == "golden_passages":
        assert Pgold is not None, f"{Pgold=}"
        trigger_init = " ".join(
            random.choices(Pgold, k=min(len(Pgold), 10))
        )  # repeat to match the trigger len
    elif adv_passage_init == "att_queries":
        assert Q is not None, f"{Q=}"
        trigger_init = " ".join(
            random.choices(Q, k=min(len(Q), 20))
        )  # repeat to match the trigger len
    else:
        raise ValueError(f"No suitable {adv_passage_init=}")

    # 4. Tokenize {1,2,3}
    embedder_passage_prefix = model.tokenizer(
        embedder_passage_prefix, add_special_tokens=False
    )["input_ids"]
    mal_info = model.tokenizer(mal_info, add_special_tokens=False)["input_ids"]
    trigger_init = model.tokenizer(trigger_init, add_special_tokens=False)["input_ids"][
        :trigger_len
    ]

    # 4'. Retokenize (decode and encode) to avoid weird tokens
    mal_info = model.tokenizer(
        model.tokenizer.decode(mal_info), add_special_tokens=False
    )["input_ids"]
    trigger_init = model.tokenizer(
        model.tokenizer.decode(trigger_init), add_special_tokens=False
    )["input_ids"][:trigger_len]

    # 4''. Fill the trigger_init if needed
    trigger_init += model.tokenizer(dummy_token, add_special_tokens=False)[
        "input_ids"
    ...
    if trigger_loc == "suffix":
        ...
        trigger_slice = slice(
            len(embedder_passage_prefix) + len(mal_info),
        ...
        trigger_slice = slice(
            len(embedder_passage_prefix), len(embedder_passage_prefix) + trigger_len
        )
    elif trigger_loc == "middle":  # can be done in a more sophisticated way
        P_adv = (
            embedder_passage_prefix
            + mal_info[: len(mal_info) // 2]
            + trigger_init
            + mal_info[len(mal_info) // 2 :]
        )
        trigger_slice = slice(
            len(embedder_passage_prefix) + len(mal_info) // 2,
    ...
    P_adv_before = model.tokenizer(
        [P_adv_before] * n_targets,
        return_tensors="pt",
        padding="max_length",
        truncation=True,
    )
    ...
    P_adv = model.tokenizer(
        [P_adv] * n_targets, return_tensors="pt", padding="max_length", truncation=True
    )
...
def evaluate_attack(
    ...
    qid_to_emb: Dict[
        str, torch.Tensor
    ],  # maps each query-id to the query embedding; (emb_dim,).
    ...
    **kwargs,
    ...
    try:
        ...
        logger.warning(
            f"Cached evaluation file not found; creating a mock results dict for the evaluation. "
    ...
    logger.info(
        f"{'<' * 10} Evaluating with the centroid of the attacked queries {'>' * 10} "
    )
    metrics.update(
        full_evaluation_with_adv_passage_vecs(
    ...
    best_query_emb = _get_best_query_emb(
        attacked_qids=attacked_qids, results=results, qid_to_emb=qid_to_emb
    )
    metrics.update(
        full_evaluation_with_adv_passage_vecs(
    ...
    logger.info(
        f"{'<' * 10} Evaluating with the (text) adv passage post-attack {'>' * 10} "
    )
    ...
    logger.info(
        f"{'<' * 10} Evaluating with the (tokens-list) adv passage post-attack {'>' * 10} "
    )
    ...
    valid_tokens_number = (
        adv_toks_after_attack_pt_input["attention_mask"].sum(dim=1).item()
    )
    ...
    if best_flu_instance_text is not None:
        ...
        emb_best_flu_instance_text = (
            model.embed(best_flu_instance_text).squeeze(0).cuda()
        )
        metrics.update(
            full_evaluation_with_adv_passage_vecs(
